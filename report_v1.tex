...
\begin{normalsize}
\textit{for}\\
fulfilment of the course “Simulation Science Laboratory”

\end{normalsize}

\vspace{0.5cm}

{\normalsize $28^{th}$ February 2016}
...
Heat is an energy that flows from higher temperature to lower temperature. Conduction is a method of heat transfer that occurs by  the movement of electrons from atom to atom .Thus, densely packed materials have higher conduction. The heat conduction in 2D can be modelled using the following partial differential equation$^{[1]}$:
\begin{equation}
...
Generalizing the non linear heat conduction equation in (1.7) to 2 dimensions, we get that the temperature evolution with the time would be governed by the heat equation in the form 
\begin{equation}
...
\end{align}
This is so called second-order Runge-Kutta method. However, to gain higher accuracy we use fourth-order Runge-Kutta method and the method is as follow,
\begin{equation}
...
Since the temperature at each grid point depends only on its neighbouring points for both implicit and Rk4 schemes, we can decompose the domain to split the work . The grid is split among the processors as shown in the Fig.\ref{fig:domain}. Each processor has a ghost row at the top and bottom which holds the grid points of neighbouring ranks. // 
...
\caption{RK4(U\_current, U\_next)}
  \KwIn{Initialized grids}
  \KwOut{Update U\_current to final temperature}
...
There at each iteration over the time, we send top row to the previous rank and receive it in the last ghost row from previous rank. And we also send the bottom row to the next rank and receive it in the top ghost row of the previous rank. This is how the domain is decomposed and the communication of data between the processors occurs.\\
...
\subsection{Gradient computation using Adjoint mode AD}
...
%    \centering
%    \includegraphics[width=0.45\textwidth]{GraDiff.png}
%    \caption{}
%    \label{fig:GradDiff}
%\end{wrapfigure}
...
\begin{wrapfigure}{r}{0.35\textwidth} %this figure will be at the right
    \centering
    \includegraphics[width=0.5\textwidth]{AppenA.png}
\end{wrapfigure}
...
%%\graphicspath{{fig/}}
%\begin{center}
%\includegraphics[scale=0.55]{AppenA.png} 
%%\caption{} \vskip 10 pt
%\label{AppenA.png}
%\end{center}
%\end{figure}
...
\newpage
\textit{The following parameters can be modified:}\\
...
\begin{tabular}{ | m{11em} | m{11cm}| m{1.8cm} | } 
\hline
\textbf{Parameter} & \textbf{Description} & \textbf{Unit}\\ 
\hline
N& Grid size in each dimension. For eg, if N = 30, 32x32 grid will be initialized including the boundary rows and columns. (Please make sure that this dimension matches with the number of values in “obs\_Tmp.txt”) & - \\ 
...
[4] http://ocw.mit.edu/courses/mathematics/18-336-numerical-methods-for-partial-differential-equations-spring-2009\\
\hspace*{\parindent}\; \;\;\:/lecture-notes/MIT18\_336S09\_lec14.pdf – Von Neumann Stability analysis
...
%\bibitem{lamport94}
%  Uwe Naumann,
%  Art of Differentiating Computer Programs.
%
%\bibitem{lamport94}
%  Alan Jeffery,
...
%\bibitem{lamport94}  
%  Steepest descent method,
%  www.math.usm.edu/lambers/mat419/lecture10.pdf 
%  
%\bibitem{lamport94}
%  Von Neumann Stability analysis,
%  http://ocw.mit.edu/courses/mathematics/18-336-numerical-methods-for-partial-differential-equations-spring-2009/lecture-notes/MIT18\_336S09\_lec14.pdf 
